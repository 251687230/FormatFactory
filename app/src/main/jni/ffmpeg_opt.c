...
static AVDictionary *strip_specifiers(AVDictionary *dict)
{
    ...
    while ((e = av_dict_get(dict, "", e, AV_DICT_IGNORE_SUFFIX))) {
        ...
        if (p)
            *p = 0;
        av_dict_set(&ret, e->key, e->value, 0,0);
        if (p)
            *p = ':';
    }
    return ret;
...
static int open_input_file(OptionsContext *o, const char *filename)
{
    ...
    if (o->nb_frame_rates) {
        ...
        if (file_iformat && file_iformat->priv_class &&
            ...
            av_dict_set(&o->g->format_opts, "framerate",
                        o->frame_rates[o->nb_frame_rates - 1].u.str, 0,0);
        }
    }
    if (o->nb_frame_sizes) {
        av_dict_set(&o->g->format_opts, "video_size", o->frame_sizes[o->nb_frame_sizes - 1].u.str, 0,0);
    }
    if (o->nb_frame_pix_fmts)
        av_dict_set(&o->g->format_opts, "pixel_format", o->frame_pix_fmts[o->nb_frame_pix_fmts - 1].u.str, 0,0);
    ...
    if (!av_dict_get(o->g->format_opts, "scan_all_pmts", NULL, AV_DICT_MATCH_CASE)) {
        av_dict_set(&o->g->format_opts, "scan_all_pmts", "1", AV_DICT_DONT_OVERWRITE,0);
        scan_all_pmts_set = 1;
    }
    ...
    if (scan_all_pmts_set)
        av_dict_set(&o->g->format_opts, "scan_all_pmts", NULL, AV_DICT_MATCH_CASE,0);
    ...
    for (i = f->ist_index; i < nb_input_streams; i++) {
        ...
        while ((e = av_dict_get(input_streams[i]->decoder_opts, "", e,
            ...
            av_dict_set(&unused_opts, e->key, NULL, 0,0);
    }
...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index)
{
    ...
    if (ost->enc) {
        ...
        if (preset && (!(ret = get_preset_file_2(preset, ost->enc->name, &s)))) {
            do  {
                ...
                *arg++ = 0;
                av_dict_set(&ost->encoder_opts, buf, arg, AV_DICT_DONT_OVERWRITE,0);
                av_free(buf);
            } while (!s->eof_reached);
            avio_closep(&s);
        }
    ...
    if (ost->enc && av_get_exact_bits_per_sample(ost->enc->id) == 24)
        av_dict_set(&ost->swr_opts, "output_sample_bits", "24", 0,0);
    ...
    if (!ost->stream_copy) {
        ...
        if (do_pass) {
            if (do_pass & 1) {
                ...
                av_dict_set(&ost->encoder_opts, "flags", "+pass1", AV_DICT_APPEND,0);
            }
            if (do_pass & 2) {
                video_enc->flags |= AV_CODEC_FLAG_PASS2;
                av_dict_set(&ost->encoder_opts, "flags", "+pass2", AV_DICT_APPEND,0);
            }
        }
        ...
        if (do_pass) {
            ...
            if (!strcmp(ost->enc->name, "libx264")) {
                av_dict_set(&ost->encoder_opts, "stats", logfilename, AV_DICT_DONT_OVERWRITE,0);
            } else {
...
loop_end:
    ...
    for (i = 0; i < o->nb_attachments; i++) {
        ...
        av_dict_set(&ost->st->metadata, "filename", (p && *p) ? p + 1 : o->attachments[i], AV_DICT_DONT_OVERWRITE,0);
        avio_closep(&pb);
    }
    ...
    if (!o->metadata_global_manual && nb_input_files){
        ...
        if(o->recording_time != INT64_MAX)
            av_dict_set(&oc->metadata, "duration", NULL, 0,0);
        av_dict_set(&oc->metadata, "creation_time", NULL, 0,0);
        av_dict_set(&oc->metadata, "company_name", NULL, 0,0);
        av_dict_set(&oc->metadata, "product_name", NULL, 0,0);
        av_dict_set(&oc->metadata, "product_version", NULL, 0,0);
    }
    if (!o->metadata_streams_manual)
        for (i = of->ost_index; i < nb_output_streams; i++) {
            ...
            if (!output_streams[i]->stream_copy) {
                av_dict_set(&output_streams[i]->st->metadata, "encoder", NULL, 0,0);
            }
        }
    ...
    for (i = 0; i < o->nb_program; i++) {
        ...
        while(*p) {
            ...
            if (!strcmp(key, "title")) {
                av_dict_set(&program->metadata, "title", p2, 0,0);
    ...
    for (i = 0; i < o->nb_metadata; i++) {
        ...
        *val++ = 0;
        ...
        if (type == 's') {
            for (j = 0; j < oc->nb_streams; j++) {
                ...
                if ((ret = check_stream_specifier(oc, oc->streams[j], stream_spec)) > 0) {
                    if (!strcmp(o->metadata[i].u.str, "rotate")) {
                        ...
                        av_dict_set(&oc->streams[j]->metadata, o->metadata[i].u.str, *val ? val : NULL, 0,0);
                    }
                } else if (ret < 0)
                    exit_program(1);
            }
            ...
            av_dict_set(m, o->metadata[i].u.str, *val ? val : NULL, 0,0);
        }
    }
...
static int opt_bitrate(void *optctx, const char *opt, const char *arg)
{
    ...
    if(!strcmp(opt, "ab")){
        av_dict_set(&o->g->codec_opts, "b:a", arg, 0,0);
        return 0;
    } else if(!strcmp(opt, "b")){
        av_log(NULL, AV_LOG_WARNING, "Please use -b:a or -b:v, -b is ambiguous\n");
        av_dict_set(&o->g->codec_opts, "b:v", arg, 0,0);
        return 0;
    }
    av_dict_set(&o->g->codec_opts, opt, arg, 0,0);
    return 0;
}
...
static int opt_profile(void *optctx, const char *opt, const char *arg)
{
    ...
    if(!strcmp(opt, "profile")){
        ...
        av_dict_set(&o->g->codec_opts, "profile:v", arg, 0,0);
        return 0;
    }
    av_dict_set(&o->g->codec_opts, opt, arg, 0,0);
    return 0;
...
static int opt_timecode(void *optctx, const char *opt, const char *arg)
{
    ...
    if (ret >= 0)
        ret = av_dict_set(&o->g->codec_opts, "gop_timecode", arg, 0,0);
    av_free(tcr);
    return ret;
}
